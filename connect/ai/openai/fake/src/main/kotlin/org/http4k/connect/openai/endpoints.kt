    ...
    "/chat/completions" bind POST to
        { request ->
            ...
            val choices = (completionGenerators[chatRequest.model] ?: ChatCompletionGenerator.LoremIpsum())(chatRequest)
                .let { if (chatRequest.logprobs == true) it.map { c -> c.withSyntheticLogProbs(chatRequest.top_logprobs ?: 1) } else it }
        ...
        logprobs = ChoiceLogProbs(
            content = tokens.map { token ->
                ...
                TokenLogProb(
                    token = token,
                    logprob = logprob,
                    bytes = token.toByteArray().map { it.toInt() and 0xFF },
                    top_logprobs = (0 until topN).map { i ->
                        TopLogProb(
